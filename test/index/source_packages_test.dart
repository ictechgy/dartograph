...
class BridgeApi {
  ...
  bool getOther(String key) => key == 'other';
}
''');
    final changed = await index.index(package.path);
    expect(
      changed.graph.nodes.keys,
      contains('package:local_bridge/bridge.dart::BridgeApi.getOther'),
    );
  });

  test('source_packages retains a generated Pigeon-style package caller', () async {
    final package = await _makePigeonPackage();
    addTearDown(() => package.delete(recursive: true));
    final outside = File(
      '${package.parent.path}/outside-generated-source.dart',
    );
    await outside.writeAsString('class OutsideGeneratedSource {}\n');
    addTearDown(() => outside.delete());
    await Link(
      '${package.path}/vendor/shared_preferences_android/lib/src/outside.dart',
    ).create(outside.path);
    await File('${package.path}/dartograph.yaml').writeAsString('''
source_packages:
...
''');
    ...
    const target =
        'package:shared_preferences_android/src/messages_async.g.dart::SharedPreferencesAsyncApi.getBool';
    expect(result.graph.nodes.keys, contains(target));
    expect(
      result.graph.nodes.keys,
      isNot(
        contains(
          'package:shared_preferences_android/src/outside.dart::OutsideGeneratedSource',
        ),
      ),
    );
    expect(
...
void main() => SharedPreferencesAsyncApi().getBool('missing', SharedPreferencesPigeonOptions());
''');
  await Directory(
...
''');
  await File(
    '${package.path}/vendor/shared_preferences_android/lib/src/messages_async.g.dart',
  ).writeAsString('''
...
class SharedPreferencesAsyncApi {
  Future<bool?> getBool(String key, SharedPreferencesPigeonOptions options) async => null;
}
''');
  final result = await Process.run(Platform.resolvedExecutable, const [
    'pub',
