...
| P2 | pub workspace·모노레포 스캔 표면 | dcq_standalone 멀티 패키지, dependency_validator workspace, dallow `--recursive` | 워크스페이스 루트에서 하위 패키지 분석 경로(단일 패키지 계약과 정합 여부 먼저 결정) | 구현([PR #124](https://github.com/ictechgy/dartograph/pull/124), 0.15.0 릴리스 대상) — 옵트인 `--workspace`가 루트·멤버를 집계하고 deps는 멤버별 pubspec에 귀속한다. 기본 모드는 기존 멤버 분석·루트 한계를 유지하며 aggregate 캐시는 분리한다. |
