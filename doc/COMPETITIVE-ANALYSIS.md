...
- 가장 큰 정직한 약점은 **증분 분석 부재**다. affected-only 실행을 이미 제품화한 모노레포
  빌드 도구와 비교하면 대형 체크아웃에서 체감 속도가 밀린다(→ `doc/RESEARCH.md` 및
  `HANDOFF-PROGRESS.md` P0).
...
- **증분 분석 미구현(self-hosted).** affected-only 실행을 제품화한 빌드 도구 대비 대형
  체크아웃에서 열위다. CI 상시 실행이라는 요구 4를 만족하려면 이 항목이 필요하다.
...
| 순위 | 항목 | 근거 | 완료 판정 |
|---|---|---|---|
| P0 | 증분 분석 + CI 게이트 | 요구 4, 대체재 대비 최대 열위 | 전체 분석 대비 실측 배수(목표 5배, 미달 시 실측 기재), CI 예시 워크플로 동작 |
| P0 | 검증 원장(append-only JSONL) + `history` 조회 | 추적 가능성 요구 | 각 실행의 입력·버전·결과가 누적되고 덮어쓰기 불가 |
| P1 | 런타임 오탐 추가 축소 | 4.2 신호 대 잡음 | 합성 픽스처 오탐 0, 미판정 사유 표기 |
| P1 | MCP 스키마·예시 문서 강화 | Knip·Serena 선점 | 클라이언트 설정 예시 + 오류 코드 표 + 재현 호출 |
| P2 | 0.9.0 릴리스 패키징 | 오픈소스 배포 | 태그·CHANGELOG·`pub publish --dry-run` 통과 |
