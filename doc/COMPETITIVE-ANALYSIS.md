...
| 순위 | 항목 | 근거 | 완료 판정 | 상태 |
|---|---|---|---|---|
| P0 | 증분 분석 + CI 게이트 | 요구 4, 대체재 대비 최대 열위 | 전체 분석 대비 실측 배수(목표 5배, 미달 시 실측 기재), CI 예시 워크플로 동작 | 증분 구현(미릴리스 — warm 7.5배·leaf 1.9배·imported 약 1.0배). `.github/workflows/impact-precheck.yml`에 PR 트리거·PR 코멘트·SARIF·증분 캐시·검증 원장 예시 작성(CI 실행 미검증, 액션 SHA 고정) |
