...
dartograph 0.14.0은 Dart SDK 3.11 이상에서 동작하는 순수 Dart 패키지다.
...
파일 전체가 생성 블록 그대로인 파일(설치가 만든 것)만 파일째 지우고, 블록
안쪽을 사용자가 고쳤거나 다른 내용이 섞여 있으면 파일은 남기고 블록만
제거한다 — 그 경계에서 어느 파일도 쓰기 전에 두 파일의 결과를 모두
계산해 부분 uninstall을 피하고, 깨진 표지는 실패(exit 2)다. 없는
파일·항목은 성공으로 넘어간다. `--force`는 생성 스크립트와
