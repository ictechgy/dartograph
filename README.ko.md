...
# Flutter 브리지 사실, 에이전트 스킬·설정, MCP 서버
...
- `schema`는 persistence `relation-use` 사실(bridge-facts v1, `target: "persistence"`)을 내서 isthmus가 Dart 코드와 [schemagraph](https://github.com/ictechgy/schemagraph)가 내보낸 SQL 카탈로그를 조인하게 한다: sqflite의 SQL·테이블·컬럼 인자, sqlite3·postgres의 SQL 인자, drift `Table` 클래스·custom 쿼리·`.drift` 파일, floor `@Entity`·`@DatabaseView`·`@Query`, 대문자 SQL 문자열 리터럴. 수신자 타입은 해석하지 않으므로 `query` 같은 흔한 메서드 이름은 해당 패키지를 import한 파일에서만 센다. 비리터럴 SQL은 동적 사실로 남고, Isar·Hive 같은 비관계 저장소와 지원 밖 SQL 패키지는 사실이 아니라 한계로 보고된다. SQL 판독기는 kartograph·cartograph 추출기의 포트라 모든 생산자가 같은 SQL을 같게 읽는다.
...
- `--incremental <dir>`은 캐시된 분석 사실을 재사용해 CI 수준의 재실행 속도를 내고, `--workspace`는 pub workspace 집계를 옵트인한다(루트 pubspec의 `workspace:` 멤버를 함께 분석하고 `deps`는 패키지별 pubspec으로 감사한다). `--record <dir>`/`history`는 실행 입력·버전·결과를 append-only 원장에 남긴다.
