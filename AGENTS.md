...
- **해석 결과 캐시는 측정한 뒤에 붙입니다.** analyzer는 파일로 남는 산출물이 없어 매 실행 해석합니다. 작은 프로젝트에서 충분히 빠르면 v0.1은 캐시 없이 갑니다. 붙인다면 cartograph의 `SourceFactsCache` 구조(내용 해시 키 + 분석기 신원)를 따릅니다
